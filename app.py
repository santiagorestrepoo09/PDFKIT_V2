  ...
  try:
      ...
      for i in range(len(List_Clientes)):
        for j in range(0,1):
          ...
          ArrayEmpresaDocumento.append(List_Clientes[i][j+3])
          ArrayClienteEmailfacturavencer.append(List_Clientes[i][j+4])
      #print(ArrayClientes)
...
def Consultar_DatosClientes(List_Clientes,empresa_codigo):
  Sql2 = (f'{"SELECT te.empresa_nombre as nombreEmpresa,	cl.cliente_nombre1, substr(now()::text,	1,	10) as fechacorte,	sum(car.cartera_saldo) as TotalEstadoCuenta,	(	select	case	when sum(car.cartera_saldo) is null then 0 else sum(car.cartera_saldo) end as ValorFacturaVencidas from tb_factura ft	left join tb_cartera car on (car.factura_codigo = ft.factura_codigo) left join tb_carteracambio tc on (tc.cartera_codigo = car.cartera_codigo) left join tb_cliente cl on (cl.cliente_codigo = car.cliente_codigo) left join tb_facturacambio fc on (ft.factura_codigo = fc.factura_codigo) where cl.cliente_codigo = "}{List_Clientes}{" 	and car.empresa_codigo ="}{empresa_codigo}{"	and now()::date-car.cartera_fechacreacion-cl.cliente_diasvencimientofactura > 0 and tc.cartera_saldo >=1), (select case when sum(car.cartera_saldo) is null then 0 else sum(car.cartera_saldo) end as ValorFacturaSinVencer from tb_factura ft left join tb_cartera car on (car.factura_codigo = ft.factura_codigo) left join tb_carteracambio tc on (tc.cartera_codigo = car.cartera_codigo) left join tb_cliente cl on (cl.cliente_codigo = car.cliente_codigo) left join tb_facturacambio fc on (ft.factura_codigo = fc.factura_codigo) where cl.cliente_codigo = "}{List_Clientes}{" and car.empresa_codigo ="}{empresa_codigo}{"and now()::date-car.cartera_fechacreacion-cl.cliente_diasvencimientofactura <= 0 and tc.cartera_saldo >=1) from tb_factura tf left join tb_cartera car on (car.factura_codigo = tf.factura_codigo) left join tb_cliente cl on (cl.cliente_codigo = car.cliente_codigo) left join tb_carteracambio tc on (tc.cartera_codigo = car.cartera_codigo) left join tb_empresa te on (te.empresa_codigo = tf.empresa_codigo) left join tb_facturacambio fc on (tf.factura_codigo = fc.factura_codigo) where car.cliente_codigo = "}{List_Clientes}{" and car.empresa_codigo ="}{empresa_codigo}{"and car.cartera_saldo >= 1 group by te.empresa_nombre, cl.cliente_nombre1"} ')
  try:
